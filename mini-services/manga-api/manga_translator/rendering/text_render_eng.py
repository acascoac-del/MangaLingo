...
def render_lines(
    ...
    # rect
    x, y, width, height = cv2.boundingRect(line_box[:, :, 3])
    if width == 0 or height == 0:
        return Image.fromarray(line_box[0:1, 0:1])
...
def render_textblock_list_eng(
    ...
    downscale_constraint: float = 0.4,
    ...
    for region in text_regions:
        ...
        # Check if the region was artificially clamped to prevent intersecting other regions
        is_clamped = getattr(region, 'enlarge_ratio', 2.0) < 1.5
        
        # Check if we fell back to a solid block mask (meaning no bubble boundaries were detected)
        is_fallback = (ballon_mask == 255).mean() > 0.95

        if is_clamped or is_fallback:
            # If clamped or fallback, the ballon_mask is a tiny narrow rectangle and does not represent the true bubble.
            # Do not restrict font size by width or area, as it needs to overflow horizontally.
            font_size_multiplier = max(min(lines_available / lines_needed, 1.0), downscale_constraint)
        else:
            # If it owns the whole bubble, use area_multiplier to perfectly fit the text inside the bubble.
            area_multiplier = np.sqrt(area_ratio / 2.0)
            font_size_multiplier = max(min(region_w / (base_length + 2*sw), lines_available / lines_needed, area_multiplier), downscale_constraint)
        
        print(f"DEBUG_FONT: '{region.translation}' | size={region.font_size}->{int(region.font_size * font_size_multiplier)} | ratio={region.enlarge_ratio:.2f} | clamped={is_clamped} | mult={font_size_multiplier:.2f} | xyxy={xyxy} | region_w={region_w} | lines_avail={lines_available} | lines_need={lines_needed}", flush=True)
        ...
        intersection = np.sum(cv2.bitwise_and(lines_map, ballon_mask))
        if intersection > 0:
            valid_lines_ratio = lines_area / intersection
        else:
            valid_lines_ratio = 4.0 # Fallback if completely disjoint

        if valid_lines_ratio > 1: # text bbox > ballon area
            # valid_lines_ratio is an area ratio, so scale linearly by its square root
            scale_factor = np.sqrt(valid_lines_ratio)
            # Do not shrink more than what's reasonable (max 2.5x shrink)
            scale_factor = min(scale_factor, 2.5)
            resize_ratio = min(resize_ratio * scale_factor, 1 / downscale_constraint)
        ...
        # Disabled post-render shrinking, as it causes extreme blurring and microscopic text.
        # The font size multiplier should already handle size constraints correctly before layout.
        # if resize_ratio != 1:
        #     textlines_image = textlines_image.resize((int(textlines_image.width / resize_ratio), int(textlines_image.height / resize_ratio)), resample=Image.LANCZOS)
