...
class MangaTranslator:
    ...
    async def _run_text_rendering(self, config: Config, ctx: Context):
        ...
        if config.render.renderer == Renderer.none:
            ...
            font_path = getattr(config, "_font_path", self.font_path) or self.font_path
            # manga2eng currently only supports horizontal left to right rendering
            if (config.render.renderer == Renderer.manga2Eng or config.render.renderer == Renderer.manga2EngPillow) and ctx.text_regions and LANGUAGE_ORIENTATION_PRESETS.get(ctx.text_regions[0].target_lang) == 'h':
