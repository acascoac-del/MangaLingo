...
class GroqTranslator(CommonTranslator):
    ...
    def parse_args(self, args):
        if hasattr(args, 'groq_key') and args.groq_key:
            self.client = groq.AsyncGroq(api_key=args.groq_key, timeout=self._TIMEOUT)
        if hasattr(args, 'groq_model') and args.groq_model:
