...
# ⚡ FIX: Some versions of googletrans omit TRANSLATE_RPC — inject it if missing
if not hasattr(urls, 'TRANSLATE_RPC'):
    urls.TRANSLATE_RPC = 'https://{host}/_/TranslateWebserverUi/data/batchexecute'
...
DEFAULT_FALLBACK_SERVICE_URLS = DEFAULT_SERVICE_URLS
...
class GoogleTranslator(CommonTranslator):
    ...
    async def _translate_query(self, from_lang: str, to_lang: str, query: str) -> Translated:
        """Translate using the legacy translate_a/single endpoint (more reliable than RPC)."""
        if not query:
            return None
        ...
        for attempt in range(3):
            try:
                # ⚡ Use legacy translate_a/single endpoint (RPC batchexecute returns 404)
                ...
                r = await self.client.get(url, params=params, timeout=10.0)
                if r.status_code != 200:
                    raise Exception(f'Google Translate HTTP {r.status_code}')

                raw = r.json()
                # raw[0] is list of [translated_chunk, original_chunk, ...]
                translated = ''.join(
                    part[0] for part in raw[0] if isinstance(part, list) and part[0]
                )
                detected_src = from_lang
                try:
                    detected_src = raw[2] or from_lang
                except Exception:
                    pass

                result = Translated(
                    src=detected_src, dest=to_lang, origin=query,
                    text=translated, pronunciation=None,
                    extra_data={}, response=r,
                )
                return result
